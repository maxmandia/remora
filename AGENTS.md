...
- For new top-level backend module files, prefer dotted suffixes for `.service.ts`, `.repository.ts`, `.router.ts`, and `.types.ts`. Use `.utils.ts` only for pure, module-specific helpers that are reused or directly test-worthy; do not put dependency-bearing logic, environment access, I/O, route wiring, or catch-all helper collections there. Avoid adding new top-level files like `.http.ts` or `.callback.ts`; keep route wiring in routers and private helpers local until repetition proves they need a named module.
