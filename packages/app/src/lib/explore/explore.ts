...
export type ExploreVhsTapeDetails<TKey extends string = string> = {
  ...
  modelId: string;
  prompt: string;
  referenceMedia?: ExploreReferenceMedia;
  resolution: string;
  title: string;
  videoUrl: string;
};

export type ExploreReferenceMedia = {
...
const seedance25ExploreGenerationDefaults = {
  duration: 30,
...
function getExploreAdsReferenceUrls({
  ...
  return Array.from(
    { length: count },
    (_, index) =>
      `${exploreAdsReferenceBaseUrl}/${tapeKey}/${kind}${index + 1}.${extension}`,
  );
}

export const exploreVhsTapes = [
  {
    ...seedance20ExploreGenerationDefaults,
    description:
...
export const exploreAdsVhsTapes = [
  ...
  {
    ...seedance25ExploreGenerationDefaults,
    description:
    ...
    referenceMedia: {
      images: getExploreAdsReferenceUrls({
        count: 1,
        kind: "image",
        tapeKey: "d84a70e3",
      }),
  ...
  },
  {
    ...
    referenceMedia: {
      ...
      videos: getExploreAdsReferenceUrls({
        count: 6,
        kind: "video",
        tapeKey: "e31cb569",
      }),
