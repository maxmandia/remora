...
type CreditCheckoutReturnInput = Pick<
  CreateCreditCheckoutSessionInput,
  "checkoutReturnTarget" | "desktopReturnUrl"
>;

export type CreditsSettingsCheckoutAdapter = {
  getReturnInput: () => Promise<CreditCheckoutReturnInput | undefined>;
...
export function CreditsSettingsPage({
  ...
  checkoutAdapter: CreditsSettingsCheckoutAdapter;
}) {
  const trpc = useTRPC();
  ...
  async function handleCheckoutSubmit(value: CreditPurchaseFormValue) {
    ...
    try {
      const checkoutReturnInput = await checkoutAdapter.getReturnInput();
      const { checkoutUrl } = await createCheckoutSessionMutation.mutateAsync({
        ...checkoutInput,
        ...checkoutReturnInput,
      });

      checkoutAdapter.openCheckout(checkoutUrl);
      handleBuyCreditsDialogOpenChange(false);
    } catch {
  ...
  return (
    <SettingsPageLayout title="Credits" description="Manage your credits.">
...
function SettingsPageLayout({
  ...
  return (
    <div className="h-full min-h-full overflow-auto px-26 py-10">
