...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE "id" = 'tripo-p1-image-to-3d'
      AND (
        "provider_id" <> 'tripo'
        OR "type" <> 'model3d'
      )
  ) THEN
    RAISE EXCEPTION 'Immutable generation model identity does not match: tripo-p1-image-to-3d';
  END IF;
END
...
BEGIN
  IF EXISTS (
    ...
    WHERE "id" = 'tripo-p1-image-to-3d-v1'
      AND (
        "model_id" <> 'tripo-p1-image-to-3d'
        OR "version" <> 1
        OR "schema_version" <> 1
        OR (
          "status" <> 'draft'
...
BEGIN
  IF EXISTS (
    ...
    WHERE "id" = 'tripo-p1-image-model-none'
      AND (
        "model_spec_id" <> 'tripo-p1-image-to-3d-v1'
    ...
    RAISE EXCEPTION 'Generation model rate identity is immutable: tripo-p1-image-model-none';
  END IF;
END
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM "generation_model_rate"
    WHERE "id" = 'tripo-p1-image-model-standard'
      AND (
        "model_spec_id" <> 'tripo-p1-image-to-3d-v1'
        OR "component" <> 'output_model3d'
        OR "quantity_source" <> 'output_model3d_count'
    ...
    RAISE EXCEPTION 'Generation model rate identity is immutable: tripo-p1-image-model-standard';
  END IF;
END
$model_rate$;--> statement-breakpoint
INSERT INTO "generation_model_rate" (
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM "generation_model_rate"
    WHERE "id" = 'tripo-p1-image-model-detailed'
      AND (
        "model_spec_id" <> 'tripo-p1-image-to-3d-v1'
        OR "component" <> 'output_model3d'
        OR "quantity_source" <> 'output_model3d_count'
    ...
    RAISE EXCEPTION 'Generation model rate identity is immutable: tripo-p1-image-model-detailed';
  END IF;
END
$model_rate$;--> statement-breakpoint
INSERT INTO "generation_model_rate" (
...
BEGIN
  IF EXISTS (
    ...
    WHERE "id" = 'tripo-p1-image-v1-concurrent'
      AND (
        "model_spec_id" <> 'tripo-p1-image-to-3d-v1'
        OR "bucket_id" <> 'tripo-p1-concurrent-task'
      )
  ) THEN
    RAISE EXCEPTION 'Generation model rate-limit identity is immutable: tripo-p1-image-v1-concurrent';
  END IF;
END
