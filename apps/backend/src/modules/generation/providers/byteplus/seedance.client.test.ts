...
describe('BytePlusSeedanceClient', () => {
  ...
  it('retrieves and normalizes failed task responses', async () => {
    ...
    await expect(client.retrieveSeedanceVideoTask('cgt-123')).resolves.toMatchObject({
      providerTaskId: 'cgt-123',
  ...
  it('normalizes HTTP errors without exposing raw response bodies', async () => {
    ...
    await expect(client.retrieveSeedanceVideoTask('cgt-123')).rejects.toMatchObject({
  ...
  it('rejects malformed JSON responses', async () => {
    const fetcher = vi.fn(async () => new Response('not json')) as unknown as FetchMock
    const client = new BytePlusSeedanceClient({
      apiKey: 'ark-test-key',
      baseUrl: 'https://ark.example.test/api/v3',
      fetcher,
    ...
    await expect(client.retrieveSeedanceVideoTask('cgt-123')).rejects.toBeInstanceOf(
      ProviderHttpError,
    )
  })
})
