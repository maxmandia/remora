...
export function buildSeedanceVideoTaskRequest({
  ...
  payloadBuilder.setProviderValue(spec.modelParameter.path, spec.providerModelId)
...
function assertSeedanceSpec(spec: VideoModelSpec) {
  if (spec.id !== seedanceModelId || spec.provider !== bytePlusProviderId) {
    throw new SeedancePayloadError('Seedance payloads require the BytePlus Seedance model spec')
  }

  if (!spec.providerModelId) {
    throw new SeedancePayloadError('Seedance model spec is missing providerModelId')
  }

  if (spec.modelParameter.source !== 'spec') {
    throw new SeedancePayloadError('Seedance model spec must provide the model parameter from spec')
  }

  if (!spec.transforms.some((transform) => transform.kind === 'seedanceContentArray')) {
    throw new SeedancePayloadError('Seedance model spec is missing the content-array transform')
  }
}
...
function validateSeedanceInput(input: CreateSeedanceVideoTaskInput) {
  ...
  if (hasDraftTask && (prompt || images.length > 0 || videos.length > 0 || audios.length > 0)) {
    throw new SeedancePayloadError('draftTaskId cannot be combined with prompt or references')
  }

  if (!hasDraftTask && !prompt && images.length === 0 && videos.length === 0) {
    throw new SeedancePayloadError('Seedance input requires a prompt, image, video, or draft task')
  }

  if (images.length > 9) {
    throw new SeedancePayloadError('Seedance supports at most 9 reference images')
  }

  if (videos.length > 3) {
    throw new SeedancePayloadError('Seedance supports at most 3 reference videos')
  }

  if (audios.length > 3) {
    throw new SeedancePayloadError('Seedance supports at most 3 reference audio files')
  }

  if (audios.length > 0 && images.length === 0 && videos.length === 0) {
    throw new SeedancePayloadError('Seedance audio references require at least one image or video reference')
  }
  ...
  if (firstFrameCount > 1) {
    throw new SeedancePayloadError('Seedance supports at most one first-frame image')
  }

  if (lastFrameCount > 1) {
    throw new SeedancePayloadError('Seedance supports at most one last-frame image')
  }

  if (lastFrameCount > 0 && firstFrameCount === 0) {
    throw new SeedancePayloadError('Seedance last-frame images require a first-frame image')
  }

  if (input.serviceTier && input.serviceTier !== 'default') {
    throw new SeedancePayloadError('Seedance 2.0 only supports the default online service tier')
  }
}

function buildSeedanceContent(input: CreateSeedanceVideoTaskInput): SeedanceContentItem[] {
